...
def ray_trace(camera, scene_settings, objects, width, height):
    ...
    for j in tqdm(range(height)):
        for i in range(width):
            camera_ray = camera.get_ray(i, j, width, height)
            color = get_color(camera_ray, scene_settings, surfaces, lights, np.array(scene_settings.background_color), materials,depth=0, max_depth=scene_settings.max_recursions,n_shadow_rays=int(scene_settings.root_number_shadow_rays))
